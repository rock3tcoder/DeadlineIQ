...
interface DashboardShellProps {
  user: UserProfile
  /** Whole days left on the free trial, or null if not on trial */
  trialDaysLeft?: number | null
  children: React.ReactNode
}

export function DashboardShell({ user, trialDaysLeft = null, children }: DashboardShellProps) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
      ...
      <main className="flex-1 md:ml-60 pt-14 md:pt-0 flex flex-col min-h-screen">
        {trialDaysLeft !== null && (
          <div className="flex items-center justify-center gap-3 border-b border-blue-500/20 bg-blue-500/10 px-4 py-2 text-center">
            <p className="text-xs text-blue-300">
              <span className="font-semibold">
                Free trial — {trialDaysLeft === 0 ? 'ends today' : `${trialDaysLeft} day${trialDaysLeft === 1 ? '' : 's'} left`}.
              </span>{' '}
              Subscribe to keep monitoring after it ends.
            </p>
            <Link
              href="/settings/billing"
              className="shrink-0 rounded-md bg-blue-600 px-2.5 py-1 text-xs font-semibold text-white hover:bg-blue-500 transition-colors"
            >
              Choose a plan
            </Link>
          </div>
